                ...
                {searchTextResult.length > 0 ? (
                    <>
                        {searchTextResult.slice(0, 2).map((product, i) => (
                            <Link className="link" to={`/productitem/${product.product_url_title}`} key={i}>
                                <div className='d-flex mb-1 hoverSearchItem'>
                                    <img className="searchImg" src={product.image_src} alt="productImg" />
                                    <div>
                        ...
                        {searchTextResult.length > 2 && <p className="linkBtn" onClick={goToSearchResultPage}>View all products</p>}
                    </>
                ) : (
            ...
            <nav className="navbar navbar-expand-lg navbar-light bg-secondary fixed-top">
                <div className="navbarInnerSection d-flex justify-content-around align-items-center">
                    ...
                    <div className="form input_group">
                        <input onChange={onChangeSearchText} value={searchText} type="text" placeholder="Search.." />
